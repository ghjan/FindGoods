...
ttk.Label(root, text='  ').grid(row=0)
# Label
ttk.Label(root, text='输入商品关键字：').grid(row=1)
# 关键字输入框
var = ttk.StringVar()
e = ttk.Entry(root, textvariable=var).grid(row=2)
# 确认按钮
ttk.Button(root, text="开始查询", command=startSpider).grid(row=3)
ttk.Button(root, text="刷新表格", command=refreshTree).grid(row=4)

ttk.Label(root, text='  ').grid(row=5, column=0)
...
vbar = ttk.Scrollbar(root, orient=ttk.VERTICAL, command=tree.yview)
...
# 说明
ttk.Label(root, text='结果输出表格文件位于项目根目录“goods.csv”').grid(row=8)
